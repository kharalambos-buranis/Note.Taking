...
namespace Note.Taking.API.Features.Notes
{
    public class GetNoteById
    {
        ...
        public sealed class Endpoint : IEndpoint
        {
            ...
            {
                app.MapGet("/notes/{id:int}", Handler)
                   .RequireAuthorization()
                   .WithTags("Notes");
            }
        }

        public static async Task<IResult> Handler(int id, ClaimsPrincipal user, AppDbContext context, ILogger<GetNoteById> logger, CancellationToken cancellationToken)
        {
